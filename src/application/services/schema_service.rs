...
use crate::infrastructure::parser::{Parser, ParseError};
use std::sync::Arc;
use thiserror::Error;
...
impl SchemaService {
    ...
    pub fn load_schema_from_file(&mut self, file_path: &str) -> Result<Arc<Schema>, SchemaServiceError> {
...
impl<'a> SchemaBuilder<'a> {
    ...
    pub fn add_type(mut self, type_def: crate::domain::entities::types::GraphQLType) -> Result<Self, SchemaServiceError> {
        self.schema.add_type(type_def)
            .map_err(|e| SchemaServiceError::ValidationError { 
                errors: vec![e] 
            })?;
        Ok(self)
    }
    ...
    pub fn add_directive(mut self, directive: crate::domain::entities::types::DirectiveDefinition) -> Result<Self, SchemaServiceError> {
...
impl IntrospectionResult {
    fn from_schema(schema: &Schema) -> Self {
        ...
        let types: Vec<TypeInfo> = schema.types.iter()
            .map(|(name, type_def)| TypeInfo::from_graphql_type(name, type_def))
            .collect();
            
        let directives: Vec<DirectiveInfo> = schema.directives.iter()
...
impl TypeInfo {
    fn from_graphql_type(name: &str, type_def: &crate::domain::entities::types::GraphQLType) -> Self {
        ...
        match type_def {
            ...
            GraphQLType::Object(obj) => Self {
                ...
                fields: Some(obj.fields.iter()
                    .map(|(field_name, field)| FieldInfo::from_field_definition(field_name, field))
                    .collect()),
                possible_types: None,
                enum_values: None,
                input_fields: None,
            ...
            GraphQLType::Interface(interface) => Self {
                ...
                fields: Some(interface.fields.iter()
                    .map(|(field_name, field)| FieldInfo::from_field_definition(field_name, field))
                    .collect()),
                possible_types: None,
                enum_values: None,
...
impl FieldInfo {
    fn from_field_definition(name: &str, field: &crate::domain::entities::types::FieldDefinition) -> Self {
        Self {
            name: name.to_string(),
...
impl DirectiveInfo {
    fn from_directive(name: &str, directive: &crate::domain::entities::types::DirectiveDefinition) -> Self {
        Self {
            name: name.to_string(),
            description: directive.description.clone(),
            locations: directive.locations.iter().map(|loc| format!("{:?}", loc)).collect(),
...
mod tests {
    ...
    fn build_schema_programmatically() {
        ...
        let result = service.build_schema()
            .query_type("Query")
            .add_type(user_type)
            .unwrap()
            .build();
