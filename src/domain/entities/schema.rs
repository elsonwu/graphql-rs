...
pub enum SchemaError {
    ...
    #[error("Argument '{argument}' in field '{field}' of type '{type_name}' has invalid type: {reason}")]
    InvalidArgumentType {
        type_name: String,
        field: String,
        argument: String,
...
impl Schema {
    ...
    pub fn get_implementations(&self, interface_name: &str) -> Vec<&ObjectType> {
        self.types.values()
            .filter_map(|t| match t {
                GraphQLType::Object(obj) if obj.interfaces.contains(&interface_name.to_string()) => Some(obj),
                _ => None,
            })
            .collect()
    }
    ...
    pub fn get_union_members(&self, union_name: &str) -> Option<Vec<&GraphQLType>> {
        if let Some(GraphQLType::Union(union_type)) = self.get_type(union_name) {
            let members: Vec<_> = union_type.types.iter()
    ...
    fn add_builtin_directives(&mut self) {
        ...
        let deprecated_directive = DirectiveDefinition {
            ...
            arguments: {
                let mut args = HashMap::new();
                args.insert("reason".to_string(), InputFieldDefinition {
        ...
        self.directives.insert("include".to_string(), include_directive);
        self.directives.insert("skip".to_string(), skip_directive);
        self.directives.insert("deprecated".to_string(), deprecated_directive);
    }
    ...
    fn validate_root_types(&self) -> Result<(), SchemaError> {
        ...
        if let Some(ref subscription_type_name) = self.subscription_type {
            match self.get_type(subscription_type_name) {
                Some(GraphQLType::Object(_)) => {},
                Some(_) => return Err(SchemaError::InvalidRootType(subscription_type_name.clone())),
    ...
    fn validate_interface_implementation(&self, obj: &ObjectType, interface: &InterfaceType) -> Result<(), SchemaError> {
    ...
    fn check_circular_references(&self) -> Result<(), Vec<SchemaError>> {
        ...
        for type_name in self.types.keys() {
            if !visited.contains(type_name) {
                if let Err(error) = self.visit_type_for_cycles(type_name, &mut visited, &mut visiting) {
                    errors.push(error);
                }
            }
        }
    ...
    fn is_type_compatible(&self, implementor_type: &GraphQLType, interface_type: &GraphQLType) -> bool {
...
mod tests {
    ...
    fn prevent_duplicate_type() {
        ...
        assert!(matches!(schema.add_type(user_type2), Err(SchemaError::DuplicateType(_))));
    }
}
