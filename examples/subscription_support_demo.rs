...
use graphql_rs::domain::{
    ...
    value_objects::{ValidationResult, GraphQLError, SubscriptionResult}
};
...
fn create_chat_schema() -> Schema {
    ...
    types.insert("Boolean".to_string(), GraphQLType::Scalar(ScalarType::Boolean));
    ...
    user_fields.insert("online".to_string(), FieldDefinition {
    ...
    message_fields.insert("id".to_string(), FieldDefinition {
        name: "id".to_string(),
    ...
    message_fields.insert("content".to_string(), FieldDefinition {
    ...
    message_fields.insert("author".to_string(), FieldDefinition {
    ...
    message_fields.insert("timestamp".to_string(), FieldDefinition {
    ...
    subscription_fields.insert("userStatusChanged".to_string(), FieldDefinition {
    ...
    subscription_fields.insert("typingIndicator".to_string(), FieldDefinition {
    ...
    mutation_fields.insert("sendMessage".to_string(), FieldDefinition {
    ...
    query_fields.insert("messages".to_string(), FieldDefinition {
    ...
    query_fields.insert("users".to_string(), FieldDefinition {
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let subscription_examples = vec![
        ("Basic Message Subscription", "subscription { messageAdded }"),
        ("User Status Subscription", "subscription { userStatusChanged }"),
        ("Typing Indicator Subscription", "subscription { typingIndicator }"),
        ("Multiple Subscriptions", "subscription { messageAdded userStatusChanged }"),
    ];
    ...
    for (title, subscription_query) in subscription_examples {
        ...
        match executor.execute(&query, &schema).await {
            ...
            result => {
                println!("   📞 Subscription requires transport: {}", result.errors[0].message);
    ...
    let error_result = SubscriptionResult::with_error(
        GraphQLError::new("Connection lost".to_string())
    );
    println!("   Error result created - has errors: {}, has stream: {}", 
             error_result.has_errors(), error_result.has_stream());
    ...
    println!("   Multi-error result - error count: {}, has stream: {}", 
             multi_error_result.errors.len(), multi_error_result.has_stream());
...
mod tests {
    ...
    async fn test_subscription_execution_demo() {
        ...
        assert!(result.errors[0].message.contains("WebSocket") || 
